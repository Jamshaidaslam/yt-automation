"""
uploader.py — Secure Channel Publisher Engine (v7.4 - RUNTIME LOCK FIX)
...
# ══════════════════════════════════════════════════════════════════════════════
# GLOBAL LOGGING AND CONTEXT ENGINE INITIALIZATION (CRITICAL RUNTIME FIX)
# ══════════════════════════════════════════════════════════════════════════════
...
# Cloudinary Setup Configuration Engine
if os.environ.get("CLOUDINARY_CLOUD_NAME"):
    ...
    logger.warning("⚠️ Cloudinary Environment parameters are missing in the pipeline matrix.")
    ...
    if token_json_str:
        try:
            ...
            if refresh_token and client_id and client_secret:
                logger.info("Rebuilding credentials from Environment Engine Secrets...")
                creds = Credentials(
                    token=None,
...
def cleanup_cloudinary(public_id: str, resource_type: str = "video"):
    try:
        ...
        logger.info(f"✅ Cloudinary cleanup complete for node: {public_id}")
    except Exception as e:
...
def upload_to_youtube(video_path: str, seo: dict, publish_at: str = None) -> str:
    ...
    try:
        if publish_at:
            ...
            logger.info(f"📅 Scheduling YouTube video execution: {publish_at}")
        else:
            privacy_status = "public"
            logger.info("📤 Publishing YouTube video instantly to short feed.")
        ...
        logger.info(f"✅ YouTube Upload complete Matrix! ID: {video_id}")
        return f"success_id_{video_id}"

    except Exception as err:
        logger.error(f"YouTube upload execution sequence failed: {err}")
...
def post_to_facebook_via_url(video_url: str, thumb_url: str, caption: str) -> str:
    try:
        ...
        if "id" in res:
            logger.info(f"✅ Facebook page payload posted! ID: {res['id']}")
            return f"success_id_{res['id']}"
        logger.error(f"Facebook Node system rejection response: {res}")
...
def post_to_instagram_via_url(video_url: str, thumb_url: str, caption: str) -> str:
    try:
        ...
        res_data    = requests.post(container_url, data=payload, headers=headers, timeout=60).json()
        ...
        for i in range(15):
            ...
            logger.info(f"Instagram validation tracking loop {i+1}/15: {status}")
            if status == "FINISHED":
                break
            elif status == "ERROR":
        ...
        if "id" in pub:
            logger.info(f"✅ Instagram Reel deployed successfully! ID: {pub['id']}")
...
def upload_all_platforms(
    ...
    logger.info(f"Initiating publisher for target file node: {video_path}")
    ...
    # 1. YouTube API Upload Route
    results["youtube"] = upload_to_youtube(video_path, seo, publish_at=publish_at)

    # 2. Cloudinary Pipeline Sync Engine for Meta Staging CDN
