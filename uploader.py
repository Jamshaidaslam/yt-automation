"""
main.py — Automated Production Executive Matrix (v10.6 - CORE ROUTER CONNECTED)
...
def execute_pipeline(topic: str = ""):
    ...
    try:
        ...
        if output_video_file.exists() and output_video_file.stat().st_size > 50000:
            ...
            # 5. FIXED SOCIAL MEDIA AUTO-UPLOAD MATRIX 🚀
            logger.info("🛰️ Connecting Central Uploader Control Node...")
            
            # Build proper matching SEO dictionary for uploader.py
            seo_payload = {
                "title": script_data.get("title", "Dark Psychology Secrets"),
                "description": script_data.get("voiceover", ""),
                "hashtags": ["#darkpsychology", "#manipulation", "#psychologyfacts", "#shorts", "#Shorts"]
            }
            
            if os.path.exists("uploader.py"):
                try:
                    logger.info("📦 Importing and binding to uploader.upload_all_platforms...")
                    import uploader
                    
                    # Target the exact working function signature
                    upload_logs = uploader.upload_all_platforms(
                        video_path=str(output_video_file),
                        seo=seo_payload,
                        thumbnail_path=None
                    )
                    logger.info(f"✅ Auto-uploader finished execution successfully. Logs: {upload_logs}")
                except Exception as up_err:
                    logger.error(f"❌ Central upload channel integration failed: {up_err}")
            else:
                logger.warning("⚠️ uploader.py was not found in root directory — skipping automatic publication.")
